...
int pc_link_rx_dma(PC_LINK_HANDLE *handle) {
  ...
  HAL_StatusTypeDef status = HAL_UARTEx_ReceiveToIdle_DMA(
      handle->huart, handle->rx_buf, handle->rx_buf_len);
...
int pc_link_tx_dma(PC_LINK_HANDLE *handle, const uint8_t *data, uint16_t len) {
  if (!handle || !handle->huart || !handle->tx_buf)
    return PC_LINK_ERROR;
  if (handle->tx_busy)
  ...
  HAL_StatusTypeDef status =
      HAL_UART_Transmit_DMA(handle->huart, handle->tx_buf, len);
  if (status != HAL_OK) {
...
void pc_link_uartex_rx_event(
    PC_LINK_HANDLE *handle, UART_HandleTypeDef *huart,
    uint16_t size) // rx event is able to change input length
{
  // Check whether it is the specified handler
  ...
  if (handle->tx_busy == 0) {
    uint16_t len = size;
    if (len > handle->tx_buf_len)
      len = handle->tx_buf_len;
