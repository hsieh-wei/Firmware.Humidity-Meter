...
int lcd_send_data(LCD_HANDLE *handle, uint8_t data) // also can using in send parameter
{
...
int lcd_init(LCD_HANDLE *handle)
{
    ...
    if (lcd_send_data(handle, 0x00) != LCD_SUCCESS) return LCD_ERROR; // memory read and display: top left => top right => bottom left => bottom right、RG
